...
RULES_PATH = (Path(__file__).parent.parent
              / "blue-team" / "detection" / "suricata" / "local.rules")
...
class TestSuricataCoverage(unittest.TestCase):
    ...
    def test_rules_file_is_non_empty(self):
        ...
        self.assertGreater(len(self.rule_msgs), 5,
                           "local.rules should have at least a handful of rules")
    ...
    def test_each_rule_has_a_classtype(self):
        ...
        for line in non_comment:
            if line.strip().startswith("alert"):
                self.assertIn("classtype:", line,
                              f"missing classtype in rule: {line[:80]}...")
