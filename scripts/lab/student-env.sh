...
# P6 (S5): per-student unique secrets. The dashboards refuse to boot on an
# unset/known-default SECRET_KEY, and the playbook runner is gated on
# PLAYBOOK_AUTH_TOKEN -- so generate fresh random values here rather than
# letting students inherit the committed placeholder.
gen_secret() {
