...
- **All 13 milestones are complete** — M1–M12 (lab infrastructure, red/blue-team content, IR playbooks, PKI lab, forensic scoreboard, plus five post-release hardening/quality phases) and M13 (Phase G security & measurement remediation). 133 issues closed, 0 open.
- **Milestone M13 — Phase G, completed 2026-08-30.** A 2026-08-13 full-repo security audit + purple-team gap analysis found 39 attacker technique/infrastructure findings against actual detection coverage (**1 Covered / 14 Partial / 24 Blind** at the time of audit). All 32 Phase G issues shipped across 35 merged PRs ([#202–#243](https://github.com/voltron-1/Adversary-in-a-box/pulls?q=is%3Apr+is%3Amerged+head%3Aphase-g)), closing the measurement trust chain (score-forgery prevention, provenance stamping, award audit trail), containment verification (compose-config test, live probe, Suricata tripwire), the Zeek/Suricata sensor tier, a pcap-replay regression harness, behavioural detections via a Falco eBPF collector, and the PKI issuance ledger. See [`docs/20260813-remediation-plan.md`](docs/20260813-remediation-plan.md) for the technical detail and [`docs/IMPLEMENTATION_PLAN.md` → Phase G](docs/IMPLEMENTATION_PLAN.md#phase-g--security--measurement-remediation) for the per-issue tracking table.
- **Audit evidence:** the raw per-module audit reports behind Phase G are in [`findings/`](findings/); the sequenced execution view is [`planned_execution.md`](planned_execution.md).
- **Next up:** no backlog is currently open. New work starts by filing issues against a new milestone.
